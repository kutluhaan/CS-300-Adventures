...
public:
    ...
    /**
     * makes the arrray full of empty
     */
    void makeEmpty(){
    ...
    void update(int currentPos, Key document){
        ...
        for (int i = 0; i < array[ currentPos ].documentInformation.size(); i++)
        {
            if (array[ currentPos ].documentInformation[i].documentName == document){
                array[ currentPos ].documentInformation[i].count++; //update the count if docuemnt name is same
                newOne = false;
                break;
            }
        }
    ...
    void rehash( )
    {
        ...
        cout << "rehashed...\n";
        cout << "previous table size: " << oldArray.size() << ", new table size: " << array.size() << ", current unique word count " << wordCount << ", current load factor: " << static_cast<double>(getInserted()) / static_cast<double>(getSize()) << endl;
    }
     ...
     */
    int getInserted() { return wordCount; }
